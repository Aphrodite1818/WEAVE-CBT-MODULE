...
const POST_SEAL_STATUSES = new Set(['active', 'suspended', 'closing', 'cancelling', 'closed', 'cancelled'])
...
export function AdminExamsPage({ adminData, gateway, onNavigate }) {
  ...
  const counts = useMemo(() => {
    const result = Object.fromEntries(PAPER_TABS.map(([key]) => [key, 0]))
    result.all = adminData.exams.length
    adminData.exams.forEach((exam) => {
      const examPhase = authoringPhase(exam.status)
  ...
  const filtered = useMemo(() => {
    ...
    return adminData.exams.filter((exam) => {
      if (phase !== 'all' && authoringPhase(exam.status) !== phase) return false
  ...
  }, [adminData.exams, componentId, phase, query, subjectId])
  ...
  const subjectOptions = [
    ...
    ...adminData.subjects.map((subject) => ({ value: subject.id, label: subject.name, description: subject.code || undefined })),
  ]
  ...
  return (
    <div className="teacher-reference-page teacher-exams-page admin-exams-page">
      ...
      <nav className="teacher-tab-row teacher-exam-tabs" aria-label="Exam preparation filters">
        {PAPER_TABS.map(([key, label]) => (
          <button key={key} type="button" className={phase === key ? 'active' : ''} aria-current={phase === key ? 'page' : undefined} onClick={() => setFilter(setPhase)(key)}>
      ...
      <section className={`exam-collection exam-collection--${view}`} aria-label="School examination papers" aria-busy={adminData.loading}>
        ...
        {!adminData.loading && visible.length === 0 && (
          <div className="teacher-exam-collection__empty">
            ...
            <strong>{adminData.exams.length ? 'No examination papers match these filters' : 'No examination papers yet'}</strong>
            <p>{adminData.exams.length ? 'Adjust the search or preparation filters.' : 'Create the first draft examination for this school.'}</p>
          </div>
        )}
...
function authoringPhase(status) {
  if (status === 'draft' || status === 'submitted' || status === 'sealed') return status
  if (POST_SEAL_STATUSES.has(status)) return 'post-seal'
  return 'post-seal'
}

function preparationLabel(status) {
  if (status === 'draft') return 'Draft authoring'
  if (status === 'submitted') return 'Administrator review'
  if (status === 'sealed') return 'Sealed paper'
  return `Post-seal · ${titleCase(status)}`
}
