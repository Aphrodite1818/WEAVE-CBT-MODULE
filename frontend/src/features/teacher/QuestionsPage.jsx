...
import { FilterBar, Notice, PageTitle, Panel, SearchField, SegmentedControl } from '../../components/ui'
  ...
  return (
    <>
      ...
      <FilterBar>
        ...
        <select aria-label="Question bank filter">
          <option>All loaded question banks</option>
          {teacherData.banks.map((bank) => <option key={bank.id}>{bank.name}</option>)}
        </select>
        <button className="button button--secondary" onClick={teacherData.refresh}>Refresh</button>
      </FilterBar>
      <Panel title="Question Library">
        {teacherData.loading ? <p>Loading questions...</p> : <QuestionRows questions={teacherData.questions} banks={teacherData.banks} />}
      </Panel>
    </>
  )
}
...
export function CreateQuestionPage({ state, dispatch, teacherData, gateway }) {
  ...
  const toggleCorrect = (id) => {
    setCorrect((current) =>
      type === 'single' ? [id] : current.includes(id) ? current.filter((item) => item !== id) : [...current, id],
    )
  }
  ...
  const saveQuestion = async () => {
    ...
    const payload = {
      ...
      options: options
        ...
        .map(([id, text]) => ({ text, is_correct: correct.includes(id) })),
    }
    ...
    try {
      if (type === 'single') {
        await gateway.questions.createSingleChoiceQuestion(bankId, payload)
      } else {
        await gateway.questions.createMultipleChoiceQuestion(bankId, payload)
      }
  ...
  if (!selectedBank) {
    return (
      <>
        <PageTitle title="Create Question" subtitle="No authorable bank is available." />
        <Notice tone="warning">The backend did not return any question bank you can author into.</Notice>
      </>
    )
  }
  ...
  return (
    <>
      <div className="teacher-page-head">
        <div>
          <button className="text-button" onClick={() => dispatch({ type: 'staff', patch: { section: 'questions' } })}>Back to questions</button>
          <PageTitle title="Create Question" subtitle="Save directly to the selected backend question bank." />
        </div>
        <div className="toolbar">
          <button className="button button--primary" disabled={saving} onClick={saveQuestion}>{saving ? 'Saving...' : 'Save Question'}</button>
        </div>
      </div>
      ...
      <div className="authoring-grid">
        <Panel title="Question content">
          <label className="field-stack">
            ...
            <select aria-label="Question Bank" value={bankId} onChange={(event) => setBankId(event.target.value)}>
        ...
        <Panel title="Question settings">
          ...
          <div className="answer-options">
            {options.map(([id, text], index) => (
              <label key={id} className="answer-option-row">
                ...
                <input
                  ...
                  value={text}
                  onChange={(event) => setOptions((current) => current.map((item, itemIndex) => itemIndex === index ? [id, event.target.value] : item))}
                />
            ...
            <button className="button button--secondary" onClick={() => setOptions((current) => [...current, [String.fromCharCode(65 + current.length), '']])}>
