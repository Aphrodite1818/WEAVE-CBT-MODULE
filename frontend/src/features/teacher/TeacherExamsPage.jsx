...
export function TeacherExamsPage({ state, dispatch, teacherData }) {
  ...
  const subjectOptions = [
    ...
    ...teacherData.subjects.map((subject) => ({ value: subject.id, label: subject.name, description: subject.code || undefined })),
  ]
  const componentOptions = [
    { value: 'all', label: 'All components' },
    ...teacherData.assessmentComponents.map((component) => ({ value: component.id, label: component.name, description: `${component.maximumScore} marks` })),
  ]
  ...
  return (
    <div className="teacher-reference-page">
      <div className="teacher-page-heading">
        ...
        <button className="teacher-primary-action" type="button" onClick={() => dispatch({ type: 'staff', patch: { section: 'create-exam' } })}><RiAddLine size={18} /> Create Exam</button>
      </div>
      ...
      <nav className="teacher-tab-row" aria-label="Exam status filters">
        {examTabs.map((tab) => <button key={tab} type="button" className={status === tab ? 'active' : ''} onClick={() => setFilter(setStatus)(tab)}>{titleCase(tab)} <span>{statusCounts[tab] || 0}</span></button>)}
      </nav>

      <div className="teacher-exam-filters">
        <label className="teacher-search-control teacher-search-control--grow"><RiSearchLine size={17} aria-hidden="true" /><input aria-label="Search exams" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1) }} placeholder="Search exams..." /></label>
      ...
      <section className="teacher-table-card" aria-busy={teacherData.loading}>
        <div className="teacher-table-wrap">
          <table className="teacher-reference-table teacher-exams-table">
            <thead><tr><th>Title</th><th>Subject</th><th>Assessment</th><th>Questions</th><th>Duration</th><th>Status</th><th>Updated</th><th><span className="sr-only">Actions</span></th></tr></thead>
            <tbody>
              {visibleExams.map((exam) => (
                <tr key={exam.id} className={state.staff.selectedExamId === exam.id ? 'is-selected' : ''}>
                  ...
                  <td>{exam.subjectName}</td><td>{exam.assessmentName}</td><td>{exam.questionCount}</td><td>{formatDuration(exam.durationMinutes)}</td>
                  <td><StatusBadge tone={statusTone(exam.status)}>{exam.statusLabel}</StatusBadge></td>
                  <td>{formatDate(exam.updatedAt)}</td>
                  <td><details className="teacher-row-menu"><summary aria-label={`More details for ${exam.title}`}>•••</summary><div><span><strong>Schedule</strong>{exam.scheduledStartAt ? formatDateTime(exam.scheduledStartAt) : 'Not scheduled'}</span><span><strong>Roster</strong>{exam.rosterCandidateCount ?? 0} candidates</span><span><strong>Paper version</strong>v{exam.authoringVersion || 1}</span></div></details></td>
                </tr>
              ))}
              {!teacherData.loading && visibleExams.length === 0 && <tr className="teacher-empty-table-row"><td colSpan="8"><div className="teacher-table-empty-content"><RiCalendarTodoLine size={24} /><span>{teacherData.exams.length === 0 ? 'No examinations are visible to you yet.' : 'No examinations match the current filters.'}</span></div></td></tr>}
              {teacherData.loading && <tr className="teacher-empty-table-row"><td colSpan="8">Loading examinations…</td></tr>}
            </tbody>
          </table>
        </div>
        <div className="teacher-table-footer"><span>{filtered.length === 0 ? '0 exams' : `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, filtered.length)} of ${filtered.length} exams`}</span><div><button type="button" disabled={page === 1} onClick={() => setPage((current) => current - 1)}>‹</button><span>{page} / {pageCount}</span><button type="button" disabled={page === pageCount} onClick={() => setPage((current) => current + 1)}>›</button></div></div>
      </section>
    </div>
  )
}
...
export function TeacherCreateExamPage({ dispatch, teacherData, gateway }) {
  ...
  const subjectBanks = useMemo(() => teacherData.banks.filter((bank) => !subjectId || bank.curriculumSubjectId === subjectId), [subjectId, teacherData.banks])
  const components = useMemo(() => teacherData.assessmentComponents.filter((component) => component.schemeId === schemeId), [schemeId, teacherData.assessmentComponents])
  ...
  const createExam = async (event) => {
    ...
    setError('')
    if (!teacherData.session?.id || !teacherData.term?.id) return setError('The current academic session and term are not available on this CBT server.')
    if (!title.trim() || !subjectId || !schemeId || !componentId || !bankId) return setError('Complete the required exam fields before creating the draft.')
    setSaving(true)
    try {
      ...
      setError(requestError.userMessage || 'Weave could not create this examination.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="teacher-reference-page">
      <div className="teacher-page-heading"><div><button className="text-button" type="button" onClick={() => dispatch({ type: 'staff', patch: { section: 'exams' } })}>Back to exams</button><div className="teacher-page-title-line"><span className="teacher-page-title-icon"><Icon name="calendar" size={27} /></span><h1>Create Examination</h1></div><p>Create a draft paper from the academic context already synchronized from Weave.</p></div></div>
      {error && <Notice tone="danger">{error}</Notice>}
      {!canCreate && !teacherData.loading && <Notice tone="warning">A current session, term, assigned subject, assessment scheme/component, and matching question bank are required before a teacher can create an exam.</Notice>}
      <form className="teacher-exam-create-card" onSubmit={createExam}>
        <div className="teacher-create-section">
          ...
          <div className="teacher-form-grid">
            ...
            <FieldSelect label="Subject"><SelectControl label="Subject" value={subjectId} options={teacherData.subjects.map((subject) => ({ value: subject.id, label: subject.name, description: subject.code || undefined }))} onChange={setSubjectId} /></FieldSelect>
            <FieldSelect label="Question bank"><SelectControl label="Question bank" value={bankId} options={subjectBanks.map((bank) => ({ value: bank.id, label: bank.name, description: `${bank.count || 0} questions` }))} onChange={setBankId} disabled={!subjectBanks.length} /></FieldSelect>
            <FieldSelect label="Assessment scheme"><SelectControl label="Assessment scheme" value={schemeId} options={teacherData.assessmentSchemes.map((scheme) => ({ value: scheme.id, label: scheme.name }))} onChange={setSchemeId} /></FieldSelect>
            <FieldSelect label="Assessment component" helper={selectedComponent ? `Maximum academic score: ${selectedComponent.maximumScore}` : ''}><SelectControl label="Assessment component" value={componentId} options={components.map((component) => ({ value: component.id, label: component.name, description: `${component.maximumScore} marks` }))} onChange={setComponentId} disabled={!components.length} /></FieldSelect>
            <FieldSelect label="Question selection"><SelectControl label="Question selection" value={selectionMode} options={[{ value: 'random', label: 'Random selection', description: 'Weave chooses from the active bank at sealing.' }, { value: 'manual', label: 'Manual selection', description: 'Teachers curate the exact paper.' }]} onChange={setSelectionMode} /></FieldSelect>
        ...
        <div className="teacher-create-footer"><div><strong>{teacherData.session?.name || 'No current session'}</strong><span>{teacherData.term?.name || 'No current term'}</span></div><div><button className="teacher-secondary-action" type="button" onClick={() => dispatch({ type: 'staff', patch: { section: 'exams' } })}>Cancel</button><button className="teacher-primary-action" type="submit" disabled={!canCreate || saving}>{saving ? 'Creating…' : 'Create Draft Exam'}</button></div></div>
      </form>
    </div>
  )
}
...
function FieldSelect({ label, helper, children }) {
  return <div className="teacher-field"><span>{label}</span>{children}{helper && <small>{helper}</small>}</div>
}
...
function formatSelectionMode(mode) { return mode === 'manual' ? 'Manual paper' : 'Random paper' }
