...
export function ExamAuthoringPage(props) {
  if (props.teacherData.loading && !props.teacherData.subjects.length) return <div className="teacher-exam-form-loading">Loading examination context...</div>;
  return <ExamAuthoringForm key={props.state?.staff?.selectedExamId || 'new'} {...props} />;
}

function ExamAuthoringForm({
  state,
  dispatch,
  teacherData,
  gateway,
  ...
  const leadName = !editing && !isAdmin ? 'You' : leadTeacherId ? 'Assigned teacher' : 'Administrator';
  const readiness = [
  ...
  const saveLead = async () => {
    setLeadSaving(true); setError(''); setLeadMessage('');
    try {
    ...
    } catch (err) { setError(err.userMessage || 'The lead author could not be updated.'); }
  ...
  return (
    <div className="teacher-reference-page teacher-exam-builder-page exam-authoring-page">
      <header className="teacher-exam-builder-header">
        <div>
          <button className="teacher-exam-back" type="button" onClick={leaveForm}><Icon name="back" size={16} /> Back to Examinations</button>
          <div className="teacher-page-title-line"><span className="teacher-page-title-icon"><Icon name="exam" size={28} /></span><h1>{readOnly ? 'Examination details' : editing ? 'Edit Examination' : 'Create Examination'}</h1></div>
        ...
        <div className="teacher-exam-builder-header__actions">
          ...
          {!readOnly && <button className="teacher-primary-action" type="submit" form="exam-authoring-form" disabled={!canSave || saving || leadSaving}><Icon name="plus" size={17} />{saving ? 'Saving...' : editing ? 'Save changes' : 'Create Draft Exam'}</button>}
        </div>
      </header>
      ...
      {!canSave && !teacherData.loading && !readOnly && <Notice tone="warning">A current session, term, subject, assessment component and matching question bank are required to create an exam.</Notice>}
      ...
      <form id="exam-authoring-form" className="teacher-exam-builder" onSubmit={saveExam}>
        <fieldset className="teacher-exam-builder__main" disabled={readOnly || saving || leadSaving}>
          <ExamSection number="1" title="Paper details" description="Define the academic context." kind="paper">
            <label className="teacher-exam-field"><span>Exam title <em>*</em></span><input aria-label="Exam title" required value={title} maxLength={255} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Computer Studies CA2" /></label>
            <FieldSelect label="Subject *"><SelectControl label="Subject" value={subjectId} options={teacherData.subjects.map((subject) => ({ value: subject.id, label: subject.name }))} onChange={(value) => { setSubjectId(value); setLeadTeacherId(''); setBankId(teacherData.banks.find((bank) => bank.curriculumSubjectId === value)?.id || ''); }} disabled={editing || readOnly} placeholder="Choose subject" /></FieldSelect>
            <FieldSelect label="Assessment scheme *"><SelectControl label="Assessment scheme" value={schemeId} options={teacherData.assessmentSchemes.map((scheme) => ({ value: scheme.id, label: scheme.name }))} onChange={(value) => { setSchemeId(value); setComponentId(teacherData.assessmentComponents.find((item) => item.schemeId === value)?.id || ''); }} disabled={readOnly} placeholder="Choose scheme" /></FieldSelect>
            <FieldSelect label="Assessment component *"><SelectControl label="Assessment component" value={componentId} options={components.map((component) => ({ value: component.id, label: component.name, description: `${component.maximumScore} marks` }))} onChange={setComponentId} disabled={readOnly || !components.length} placeholder="Choose component" /></FieldSelect>
          </ExamSection>
          <ExamSection number="2" title="Authoring ownership" description="Coordinate this shared paper." kind="ownership">
            {isAdmin ? <LeadAuthorControl key={`${subjectId}:${editingExam?.termId || teacherData.term?.id}`} gateway={gateway} subjectId={subjectId} termId={editingExam?.termId || teacherData.term?.id} value={leadTeacherId} onChange={setLeadTeacherId} disabled={readOnly} /> : <div className="exam-lead-identity"><span>{(actor?.display_name || 'You').slice(0, 1)}</span><div><small>Lead author</small><strong>{!editing || !readOnly ? 'You' : leadName}</strong></div></div>}
            <p className="exam-ownership-help"><RiInformationLine size={18} />{isAdmin ? 'You may assign an eligible teacher to coordinate authoring.' : 'Teachers share the paper. The lead coordinates its details and submission.'}</p>
            <div className="exam-ownership-status"><StatusBadge tone="info">Shared {editingExam?.statusLabel.toLowerCase() || 'draft'}</StatusBadge>{editing && isAdmin && !readOnly && <button type="button" className="exam-text-action" onClick={saveLead} disabled={leadSaving || leadTeacherId === (editingExam.leadTeacherId || '')}>{leadSaving ? 'Updating...' : 'Update lead'}</button>}{leadMessage && <small role="status">{leadMessage}</small>}</div>
          </ExamSection>
          <ExamSection number="3" title="Exam appearance" description="Folder colour preview." kind="appearance">
            <div className="exam-palette" role="group" aria-label="Folder colour preview">
              {FOLDER_COLORS.map(([name, color]) => <button key={name} type="button" title={name} aria-label={`${name} folder`} aria-pressed={folderColor === color} style={{ '--swatch': color }} onClick={() => setFolderColor(color)}>{folderColor === color && <RiCheckLine size={18} />}</button>)}
            </div>
            <div className="exam-folder-preview"><ExamFolder color={folderColor} size={52} /><p><strong>Preview only</strong><small>Folder colours are not saved yet.</small></p></div>
          </ExamSection>
          <ExamSection number="4" title="Questions" description="Choose the source and selection method." kind="questions">
            <FieldSelect label="Question bank *"><SelectControl label="Question bank" value={bankId} options={subjectBanks.map((bank) => ({ value: bank.id, label: bank.name }))} onChange={setBankId} disabled={editing || !subjectBanks.length || readOnly} placeholder="Choose bank" /></FieldSelect>
            <label className="teacher-exam-field"><span>Number of questions <em>*</em></span><input aria-label="Number of questions" type="number" min="1" required value={questionCount} disabled={editing} onChange={(event) => setQuestionCount(event.target.value)} /><small>{questionCount || 0} required &middot; <b>{activeBankQuestions} available</b></small>{capacityIssue && <small className="teacher-exam-field__error">{capacityIssue}</small>}</label>
            <fieldset className="teacher-exam-selection" disabled={editing || readOnly}><legend>Question selection method</legend><div><SelectionCard value="random" current={selectionMode} onChange={setSelectionMode} title="Random selection" description="Select from the bank." /><SelectionCard value="manual" current={selectionMode} onChange={setSelectionMode} title="Manual selection" description="Choose specific questions." /></div></fieldset>
          ...
          <ExamSection number="5" title="Exam settings" description="Configure delivery for students." kind="settings">
            <label className="teacher-exam-field"><span>Duration <em>*</em></span><div className="teacher-exam-input-suffix"><input aria-label="Duration in minutes" type="number" min="1" required value={durationMinutes} onChange={(event) => setDurationMinutes(event.target.value)} /><span>minutes</span></div></label>
          ...
          <ExamSection number="6" title="Schedule" description="Optional - this can be set later." kind="schedule">
            <label className="teacher-exam-field"><span>Scheduled start <small>(optional)</small></span><input aria-label="Scheduled start" type="datetime-local" value={scheduledStartAt} onChange={(event) => setScheduledStartAt(event.target.value)} /></label>
            <label className="teacher-exam-field"><span>Latest normal start <small>(optional)</small></span><input aria-label="Latest normal start" type="datetime-local" value={latestNormalStartAt} min={scheduledStartAt || undefined} onChange={(event) => setLatestNormalStartAt(event.target.value)} /></label>
            <label className="teacher-exam-field teacher-exam-field--wide"><span>Student instructions <small>(optional)</small></span><textarea aria-label="Student instructions" rows="3" value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder="Instructions students will see before they start." /></label>
          </ExamSection>
        </fieldset>
        <aside className="exam-summary" aria-label="Draft examination summary">
          ...
          <h3>{title.trim() || 'Untitled examination'}</h3><p className="exam-summary__context">{selectedSubject?.name || 'Choose subject'} &middot; {selectedComponent?.name || 'Choose component'}</p>
          ...
          {!readOnly && <div className="exam-readiness"><h3>Draft readiness</h3><p>Required to save your draft.</p><ul>{readiness.map(([label, ready]) => <li key={label} className={ready ? 'is-ready' : ''}>{ready ? <RiCheckboxCircleFill size={17} /> : <RiCheckboxBlankCircleLine size={17} />}<span>{label}</span></li>)}<li><RiCheckboxBlankCircleLine size={17} /><span>Schedule & instructions optional</span></li></ul></div>}
        </aside>
      </form>
    </div>
  );
...
const FOLDER_COLORS = [['Rose', '#cf4564'], ['Blue', '#397fd6'], ['Green', '#279b70'], ['Amber', '#b77915'], ['Purple', '#8855cd'], ['Magenta', '#b942a5'], ['Teal', '#148e96'], ['Slate', '#8190a5']];
