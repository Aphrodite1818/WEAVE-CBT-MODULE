...
class ExamAcademicIntegrityTests(unittest.IsolatedAsyncioTestCase):
    async def test_exam_term_must_belong_to_exam_session(self) -> None:
        ...
        with (
            patch.object(AcademicRepository, "get_session_by_id", AsyncMock(return_value=SimpleNamespace(id=exam.session_id))),
            patch.object(
                AcademicRepository,
                "get_term_by_id",
    ...
    async def test_component_must_belong_to_exam_scheme(self) -> None:
        ...
        with (
            ...
            patch.object(
                AcademicRepository,
                "get_term_by_id",
                AsyncMock(return_value=SimpleNamespace(academic_session_id=exam.session_id)),
            ),
            patch.object(AcademicRepository, "get_curriculum_subject_by_id", AsyncMock(return_value=curriculum_subject)),
            patch.object(AcademicRepository, "get_curriculum_by_id", AsyncMock(return_value=curriculum)),
            patch.object(AcademicRepository, "get_assessment_scheme_by_id", AsyncMock(return_value=SimpleNamespace(status="active"))),
            patch.object(
                AcademicRepository,
                "get_component_by_id",
                AsyncMock(
    ...
    async def test_target_resolution_freezes_offering_and_assignment(self) -> None:
        level_id = uuid4()
        teacher_id = uuid4()
        target = SimpleNamespace(class_id=uuid4(), subject_offering_id=None, teacher_assignment_id=None)
        exam = SimpleNamespace(id=uuid4(), term_id=uuid4(), curriculum_subject_id=uuid4())
        ...
        with (
            patch.object(ExamRepository, "list_target_classes_for_exam", AsyncMock(return_value=[target])),
            patch.object(
                AcademicRepository,
                "get_class_by_id",
                AsyncMock(return_value=SimpleNamespace(is_active=True, academic_level_id=level_id)),
            ),
            patch.object(AcademicRepository, "get_offering_for_class_scope", AsyncMock(return_value=offering)),
            patch.object(
                AcademicRepository,
                "get_active_assignment_for_class_curriculum_subject",
                AsyncMock(return_value=assignment),
            ),
            patch.object(ExamRepository, "save_target_class", AsyncMock(return_value=target)),
        ):
    ...
    async def test_teacher_cannot_target_another_teachers_class(self) -> None:
        ...
        with (
            ...
            patch.object(AcademicRepository, "get_offering_for_class_scope", AsyncMock(return_value=SimpleNamespace(id=uuid4()))),
            patch.object(
                AcademicRepository,
                "get_active_assignment_for_class_curriculum_subject",
                AsyncMock(return_value=SimpleNamespace(id=uuid4(), teacher_membership_id=uuid4())),
            ),
        ):
    ...
    async def test_exam_question_bank_must_match_curriculum_subject(self) -> None:
        exam = SimpleNamespace(id=uuid4(), curriculum_subject_id=uuid4(), maximum_score=Decimal("10"))
        snapshot = SimpleNamespace(source_question_id=uuid4(), source_question_version=2)
        source_question = SimpleNamespace(bank_id=uuid4(), is_active=True, version=2)
        with (
            patch.object(ExamRepository, "list_exam_questions", AsyncMock(return_value=[snapshot])),
            patch.object(QuestionRepository, "get_question_by_id", AsyncMock(return_value=source_question)),
            patch.object(
                QuestionRepository,
                "get_bank_by_id",
                AsyncMock(return_value=SimpleNamespace(is_active=True, curriculum_subject_id=uuid4())),
            ),
        ):
    ...
    async def test_seal_freezes_component_ids_and_maximum(self) -> None:
        ...
        with (
            patch.object(ExamRepository, "get_exam_by_id", AsyncMock(return_value=exam)),
            patch.object(ExamService, "_get_active_actor", AsyncMock(return_value=actor)),
            patch.object(
                ExamService,
                "_validate_academic_scope",
                AsyncMock(return_value=(SimpleNamespace(), curriculum, scheme, component)),
            ),
            patch.object(ExamService, "_resolve_target_provenance", AsyncMock(return_value=[])),
            patch.object(ExamService, "_validate_question_scope_and_points", AsyncMock(return_value=None)),
            patch.object(ExamRepository, "save_exam", AsyncMock(return_value=exam)),
        ):
            sealed = await ExamService.seal_exam(db, exam_id=exam.id, actor_id=actor_id, now=now)
        ...
        self.assertEqual(sealed.source_assessment_component_maximum_score, Decimal("20"))
...
class CandidateAcademicIntegrityTests(unittest.IsolatedAsyncioTestCase):
    async def test_candidate_must_be_eligible_for_target_offering(self) -> None:
        ...
        with (
            patch.object(ExamRepository, "get_exam_by_id", AsyncMock(return_value=exam)),
            patch.object(AcademicRepository, "get_enrollment_by_id", AsyncMock(return_value=enrollment)),
            patch.object(
                ExamRepository,
                "get_target_class",
                AsyncMock(return_value=SimpleNamespace(subject_offering_id=offering_id)),
            ),
            patch.object(AcademicRepository, "enrollment_is_eligible_for_offering", AsyncMock(return_value=False)),
        ):
...
class QuestionAuthorizationIntegrityTests(unittest.IsolatedAsyncioTestCase):
    async def test_teacher_with_any_assignment_can_author_shared_curriculum_bank(self) -> None:
        actor_id = uuid4()
        membership_id = uuid4()
        curriculum_subject_id = uuid4()
        ...
        with (
            patch("app.domains.questions.service.AuthRepository.get_actor_by_id", AsyncMock(return_value=actor)),
            patch.object(
                AcademicRepository,
            ...
            patch.object(
                ...
                AsyncMock(return_value=SimpleNamespace(id=membership_id, status="active")),
            ),
            patch.object(
                AcademicRepository,
