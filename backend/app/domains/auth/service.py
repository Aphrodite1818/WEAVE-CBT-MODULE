...
class LocalAuthService:
    ...
    async def login_staff(
        ...
        *,
        ...
        if (
            weave_actor.role == "teacher"
            and weave_actor.membership_id is None

        ):
            raise WeaveContractError(
                "Weave returned a teacher without a membership"
            )


        if (
            weave_actor.role == "admin"
            and weave_actor.membership_id is not None
        ):
            raise WeaveContractError(
        ...
        async with db.begin():
            ...
            actor_session = await AuthRepository.add_session(
                db,
                actor_session
            )
    ...
    async def _upsert_actor(
        ...
        *,
        ...
        actor = (
            await AuthRepository.get_actor_by_weave_identity(
                db,
                weave_actor_id,
                weave_actor.role,
                lock = True
