...
- Hardware virtualization enabled, approximately 8 GB installed RAM (7.5 GB usable), and at least 20 GB free space for the runtime, images and school data.
...
The server runs in the dedicated `WeaveCBT` distribution. A detached, singleton WSL client keeps it alive after the manager closes. Windows port forwarding is refreshed on startup, repair and a healthy status refresh. Only the web port is exposed on the selected physical LAN address; the database and Redis stay internal.
